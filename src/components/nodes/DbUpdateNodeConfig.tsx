...
const DbUpdateNodeConfig: React.FC = () => (
  <>
    ...
    <Form.List name="where">
      {(fields, { add, remove }) => (
        <>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>更新条件（where）</div>
          <Table
    ...
    <Form.List name="fields">
      {(fields, { add, remove }) => (
        <>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>设置更新数据</div>
          <Table
            dataSource={fields.map((f, idx) => ({ key: f.key }))}
            pagination={false}
            size="small"
    ...
    <Form.Item name="updateResultVarName" label="更新结果变量名" initialValue="update_data_rows">
      <Input addonBefore="$" placeholder="返回更新数据行数，失败为false，无变化为0" />
    </Form.Item>
    <Form.Item name="desc" label="备注"> <Input /> </Form.Item>
  </>
);

export default DbUpdateNodeConfig; 
